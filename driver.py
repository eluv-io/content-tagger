...
def get_auth(config: str, qhit: str) -> str:
    cmd = f"elv content token create {qhit} --update --config {config}"
...
def get_write_token(qhit: str, config: str) -> str:
    ...
    cmd = f"elv content edit {qhit} --config {config}"
    out = subprocess.run(cmd, shell=True, check=True,
                         capture_output=True).stdout.decode("utf-8")
...
def get_status(qhit: str, auth: str):
    res = requests.get(f"{server}/{qhit}/status",
...
def tag(contents: list, auth: str, assets: bool, params: dict, start_time: float = None, end_time: float = None):
    ...
    for i, qhit in enumerate(contents):
        ...
        if "llava" in params["features"]:
            params["features"]["llava"]["model"]["model"] = llama_models[i %
                                                                         len(llama_models)]
...
def commit(write_token: str, config: str):
    cmd = f"elv content finalize {write_token} --config {config}"
    out = subprocess.run(cmd, shell=True, check=True,
                         capture_output=True).stdout.decode("utf-8")
    print(out)
...
def write(qhit: str, config: str, do_commit: bool, force=False, leave_open=False):
    ...
    resp = requests.post(write_url, params={
                         "write_token": write_token, "replace": "true", "leave_open": leave_open})
...
def aggregate(qhit: str, config: str, do_commit: bool):
    ...
    resp = requests.post(aggregate_url, params={
                         "write_token": write_token, "replace": "true"})
    respdict = response_force_dict(resp)
    print(respdict)
    if do_commit and "error" not in respdict:
        commit(write_token, config)
    ...
    for feature in features:
        ...
        try:
            ...
            if res.status_code == 200:
                print(
                    f"Successfully stopped tagging for {qhit} on track {feature}.")
            else:
                print(
                    f"Failed to stop tagging for {qhit} on track {feature}: {res.status_code} {res.text}")
        except Exception as e:
            print(
                f"Error while stopping tagging for {qhit} on track {feature}: {e}")
    ...
    while True:
        try:
            if tty:
                ...
                # Wait for user input
                user_line = get_input(
                    f"{time.strftime('%Y-%m%d %H:%M:%S')} {server} > ", timeout=timeout)
            else:
                user_line = input("")
            ...
            if user_input in ["statusall", "sall"]:
                ...
                for qhit in contents:
                    if len(user_input) > 1:
                        if not re.search(user_input[1], qhit):
                            continue
                ...
                if len(user_split) > 2:
                    track = user_split[2]
                    this_tag_config['features'] = {
                        track: tag_config['features'][track]}
                contentsub = [x for x in contents if iqsub ==
                              None or re.search(iqsub, x)]
...
def quick_status(auth, qhit, filter=None):
    ...
    for imgorvid, models in status.items():
        for model, stat in models.items():
            line = "[%9s] %-32s / %s: %s" % (stat.get(
                "tagging_progress", ""), qhit, f"({imgorvid}) {model}", stat.get("status", "??"))
            if filter is None or re.search(filter, line):
                print(line)
...
if __name__ == "__main__":
    ...
    group.add_argument("-c", "--contents",
                       help="filename with list of contents (iq's) to tag")
    group.add_argument("-q", "--iq", nargs='*', default=[],
                       help="content (iq) to tag (specified directly)")
    parser.add_argument("--assets", action="store_true",
                        help="if set, tag assets instead of videos")
    parser.add_argument(
        "--config", help="fabric config file to use for making tokens")
    parser.add_argument("--tag-config", default="",
                        help="Tagger config json.  Use @ to read a file")
    # parser.add_argument("--audio-stream", default="audio", help="which audio stream to tag")
    parser.add_argument("--commit", "--finalize", action="store_true",
                        help="if set, commit (finalize) on fabric after writing on tagger")
    parser.add_argument(
        "--start-time", help="start time in seconds", default=0)
