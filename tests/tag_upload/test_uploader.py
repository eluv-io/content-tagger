...
def test_upload_tags(upload_session, get_tag):
    tags = [
        get_tag(model_track="", data="hello world"),
        get_tag(model_track="pretty", data="test tag", start_time=100, end_time=200)
    ]
    ...
    assert speech_tag.data == "hello world"

    pretty_tag = ts.find_tags(q=upload_session.dest_q, track="pretty")[0]
    assert pretty_tag.data == "test tag"
...
def test_upload_report(upload_session, get_tag):
    tags = [
        get_tag(model_track="asr", data="hello world"),
    ]
...
def test_reupload_replaces_tags_for_source(upload_session, track_resolver, get_tag):
    ...
    old_tags = [
        get_tag(model_track="asr", data="old tag 1", source_media="source1"),
        get_tag(model_track="asr", data="old tag 2", source_media="source1"),
        get_tag(model_track="asr", data="old tag 3", source_media="source2"),
    ]
    upload_session.upload_tags(tags=old_tags, tagged_sources=["source1", "source2"])

    assert {t.data for t in ts.find_tags(q=q)} == {"old tag 1", "old tag 2", "old tag 3"}
    ...
    new_tags = [
        ...
        get_tag(model_track="asr", data="new tag 2", source_media="source1"),
    ]
    ...
    assert {t.data for t in ts.find_tags(q=q, sources=["source1"])} == {"new tag 1", "new tag 2"}
    assert {t.data for t in ts.find_tags(q=q, sources=["source2"])} == {"old tag 3"}
...
def test_tags_survive_when_progress_lags_across_ticks(upload_session, get_tag):
    ...
    upload_session.upload_tags(
        tags=[get_tag(model_track="", data="word", source_media="source1")],
        tagged_sources=[],
    )
    # tick 2: auto_captions tags for source1 arrive together with its progress
    upload_session.upload_tags(
        tags=[get_tag(model_track="auto_captions", data="sentence", source_media="source1")],
        tagged_sources=["source1"],
    )

    assert {t.data for t in ts.find_tags(q=q, track="speech_to_text")} == {"word"}
    assert {t.data for t in ts.find_tags(q=q, track="auto_captions")} == {"sentence"}
...
def test_two_tracks_two_sources_with_batched_progress(upload_session, get_tag):
    ...
    upload_session.upload_tags(
        tags=[
            get_tag(model_track="", data="word1", source_media="source1"),
            get_tag(model_track="", data="word2", source_media="source2"),
        ],
        tagged_sources=[],
    )
    upload_session.upload_tags(
        tags=[
            get_tag(model_track="auto_captions", data="sentence1", source_media="source1"),
            get_tag(model_track="auto_captions", data="sentence2", source_media="source2"),
        ],
        tagged_sources=["source1", "source2"],
    )

    assert {t.data for t in ts.find_tags(q=q, track="speech_to_text")} == {"word1", "word2"}
    assert {t.data for t in ts.find_tags(q=q, track="auto_captions")} == {"sentence1", "sentence2"}
...
def test_incremental_tags_for_same_pair_are_not_wiped(upload_session, get_tag):
    ...
    upload_session.upload_tags(
        tags=[get_tag(model_track="", data="first", source_media="source1")],
        tagged_sources=["source1"],
    )
    upload_session.upload_tags(
        tags=[get_tag(model_track="", data="second", source_media="source1")],
        tagged_sources=["source1"],
    )

    assert {t.data for t in ts.find_tags(q=q, track="speech_to_text")} == {"first", "second"}
...
def test_delete_by_source_is_scoped_to_model(upload_session, track_resolver, get_tag):
    ...
    upload_session.upload_tags(
        tags=[get_tag(model_track="track_a", data="A tag", source_media="source1")],
        tagged_sources=["source1"],
    )
    ...
    assert {t.data for t in ts.find_tags(q=q, track="track_a")} == {"A tag"}
    assert {t.data for t in ts.find_tags(q=q, track="track_b")} == {"B tag"}
...
def test_reupload_replaces_tags_no_model_track(upload_session, track_resolver, get_tag):
    ...
    upload_session.upload_tags(
        tags=[get_tag(model_track="", data="old tag", source_media="source1")],
        tagged_sources=["source1"],
    )
    # asr's first configured track is speech_to_text
    assert {t.data for t in ts.find_tags(q=q, track="speech_to_text")} == {"old tag"}
    ...
    assert {t.data for t in ts.find_tags(q=q, track="speech_to_text")} == {"new tag"}
...
def test_processed_sources_without_tags_clears_configured_tracks(upload_session, track_resolver, get_tag):
    ...
    assert {t.data for t in ts.find_tags(q=q, track="speech_to_text")} == {"stale tag"}
...
def test_configured_track_deletion_respects_suffix(track_resolver, mock_q, filesystem_tagstore, get_tag):
    ...
    session.upload_tags(
        tags=[get_tag(model_track="", data="stale tag", source_media="source1"),
              get_tag(model_track="another_track", data="stale tag 2", source_media="source2")],
        tagged_sources=["source1"],
    )
    assert {t.data for t in ts.find_tags(q=q, track="speech_to_text_v2")} == {"stale tag"}
    assert {t.data for t in ts.find_tags(q=q, track="another_track_v2")} == {"stale tag 2"}
    ...
    session2.upload_tags(tags=[get_tag(model_track="another_track", data="new tag 2", source_media="source2")], tagged_sources=[])
