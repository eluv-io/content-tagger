...
def test_fetch_assets_with_preserve_track(
    ...
    job = tagstore.create_batch(
        qhit=assets_content_with_tags_clean.qhit,
    ...
    tagstore.upload_tags(tags, batch_id, q=assets_content_with_tags_clean)
    ...
    worker4 = fetcher.get_session(assets_content_with_tags_clean, req4)
    result4 = worker4.download()
    ...
    for asset in selected_assets:
        ...
        newtags.append(tag)
    tagstore.upload_tags(newtags, new_job.id, q=assets_content_with_tags_clean)

    # Verify that the tags were uploaded correctly
    uploaded_tags = tagstore.find_tags(batch_id=new_job.id, q=assets_content_with_tags_clean)
    ...
    worker5 = fetcher.get_session(assets_content_with_tags_clean, req5)
    result5 = worker5.download()
